...
export const radarEntries: RadarEntry[] = [
...

  {
    ...
    movement: 'new',
    since: '2026-09',
    tagline: 'Files in a bucket, behaving like a database table.',
    what: 'An open table format. Metadata files track which data files belong to a table at each snapshot, which buys atomic commits, schema and partition evolution, and time travel over plain Parquet in object storage — without a database server owning the data.',
    why: 'The substrate under every layer of the lake, on Ceph\'s S3 gateway. A bad transformation becomes a rollback to yesterday\'s snapshot rather than a restore, and the engine reading a table is a choice I can change later rather than a decision baked into the storage.',
    watch: [
      'Snapshots accumulate. Expiry and orphan-file cleanup are maintenance jobs you have to schedule, not features that happen.',
      'The catalog is the part that matters and the part tutorials skip. Decide where table metadata lives before writing any data.',
      'Small files are the classic failure. Compaction is not optional on streaming ingest.',
    ],
    links: [
      { label: 'iceberg.apache.org', href: 'https://iceberg.apache.org/docs/latest/' },
    ],
    related: ['apache-airflow', 'apache-ranger', 'ceph', 'duckdb'],
  },
  {
    slug: 'apache-airflow',
    name: 'Apache Airflow',
    ...
    movement: 'new',
    since: '2026-09',
    tagline: 'One DAG per hop, so a failure is local.',
    what: 'A workflow orchestrator. Pipelines are Python-defined directed graphs of tasks, with scheduling, dependency resolution, retries and backfill, and a UI that shows which run of which task failed and why.',
    why: 'Runs every transformation between lake layers. One DAG per hop rather than one enormous pipeline: a failed silver job does not block a bronze ingest, and backfilling one day is a task re-run rather than replaying everything.',
    watch: [
      'The scheduler is a real service with a real database behind it. This is not a cron replacement you forget about.',
      'Tasks that do the work in-process will happily eat the worker. Push compute to where the data is.',
      'A DAG file that does work at parse time gets executed on every scheduler loop. This bites everybody once.',
    ],
    links: [
      { label: 'airflow.apache.org', href: 'https://airflow.apache.org/docs/' },
    ],
    related: ['apache-iceberg', 'postgresql', 'kubernetes'],
  },
  {
    slug: 'apache-ranger',
    name: 'Apache Ranger',
    ...
    movement: 'new',
    since: '2026-09',
    tagline: 'One place that decides who reads which column.',
    what: 'A centralised authorisation layer for data platforms: row filters, column masking and table-level policy defined once and enforced by plugins inside each engine, with an audit trail of every decision including the denials.',
    why: 'Holds the lake policy in one place instead of in each engine\'s own config. The layer boundary and the permission boundary end up as the same line, which is the only reason the policy stays small enough to read in one sitting.',
    watch: [
      'Enforcement lives in the engine plugin. An engine without one reads straight past the policy — know which of yours are covered.',
      'It expects a stack that is heavier than a homelab. Running it here is practice, not proportionality.',
      'Masking is not encryption. It stops a query seeing a value; it does not stop someone with the files.',
    ],
    links: [
      { label: 'ranger.apache.org', href: 'https://ranger.apache.org/' },
    ],
    related: ['apache-iceberg', 'apache-airflow', 'vault'],
  },

  {
    ...
    movement: 'new',
    since: '2026-09',
    tagline: 'For the job that never finishes.',
    what: 'A stream processor with real state: event-time windows, exactly-once semantics through checkpointing, and operators that keep working state across restarts rather than recomputing from scratch.',
    why: 'Handles the streaming ingest off Kafka into the bronze layer. Streaming is the case where the job runs forever and has to survive a restart without losing its place, which is precisely what batch tooling is bad at.',
    watch: [
      'Checkpointing needs durable storage and tuning. Untuned, it is the reason a job that "works" falls over after two days.',
      'Event time versus processing time is the whole subject. Late-arriving sensor data makes that distinction expensive to get wrong.',
      'It is by far the heaviest thing in this pipeline. Only use it where the stream genuinely never ends.',
    ],
    links: [
      { label: 'flink.apache.org', href: 'https://nightlies.apache.org/flink/flink-docs-stable/' },
    ],
    related: ['apache-kafka', 'apache-iceberg', 'apache-airflow'],
  },
  {
    slug: 'apache-spark',
    name: 'Apache Spark',
    ...
    movement: 'new',
    since: '2026-09',
    tagline: 'Kept for the heavy rewrites, and nothing lighter.',
    what: 'A distributed batch engine. Work is split across executors, with a query optimiser and mature Iceberg support including the table maintenance procedures — compaction, snapshot expiry, orphan-file cleanup.',
    why: 'Earns its place on the large silver-to-gold rewrites and on compaction, which is the one job that genuinely wants more than one machine. Everything smaller goes to PyIceberg, because standing up executors to move a few hundred megabytes is theatre.',
    watch: [
      'The startup cost is real. For small tables the cluster spends longer starting than working.',
      'Its Iceberg maintenance procedures are the most complete of any engine, which is most of why it stays.',
      'Memory tuning is a craft. The defaults assume a cluster you do not have.',
    ],
    links: [
      { label: 'spark.apache.org', href: 'https://spark.apache.org/docs/latest/' },
    ],
    related: ['apache-iceberg', 'pyiceberg', 'apache-airflow'],
  },
  {
    slug: 'pyiceberg',
    name: 'PyIceberg',
    ...
    movement: 'new',
    since: '2026-09',
    tagline: 'The one that turned out to do most of the work.',
    what: 'A Python implementation of the Iceberg spec. It talks to the catalogue and the data files directly — reading, appending, overwriting and expiring snapshots — with no JVM and no cluster involved.',
    why: 'At this scale most hops are a few hundred megabytes, and a Python process that reads a table and writes the next one beats every distributed alternative on both latency and on how long it takes to debug. It pairs naturally with DuckDB for the compute in between.',
    watch: [
      'It does not implement everything Spark does. Check the feature matrix before assuming a maintenance operation exists.',
      'Being easy to reach for is how a "small job" quietly grows past the memory of one pod. Watch for that boundary.',
      'It is the newest of the three engines here, and the API still moves.',
    ],
    links: [
      { label: 'py.iceberg.apache.org', href: 'https://py.iceberg.apache.org/' },
    ],
    related: ['apache-iceberg', 'duckdb', 'apache-spark'],
  },
  {
    slug: 'soda',
    name: 'Soda',
    ...
    movement: 'new',
    since: '2026-09',
    tagline: 'A failed check stops the promotion. That is the whole feature.',
    what: 'Data quality as declarative checks — row counts, null and duplicate thresholds, freshness, schema and value distributions — written in YAML against a table and run as a step in a pipeline.',
    why: 'The gate between lake layers. Checks run against the output of a hop before it is promoted, so a null flood or a schema drift fails the run instead of quietly becoming gold-layer truth.',
    watch: [
      'A check that only warns is a check nobody reads. Failing the DAG task is the point.',
      'Thresholds need a baseline first. Setting them before you know the normal shape of the data produces noise and then apathy.',
      'It tells you the data is wrong, never why. That part is still yours.',
    ],
    links: [
      { label: 'Soda docs', href: 'https://docs.soda.io/' },
    ],
    related: ['apache-airflow', 'apache-iceberg'],
  },
  {
    slug: 'apache-kafka',
    name: 'Apache Kafka',
    ...
    movement: 'new',
    since: '2026-09',
    tagline: 'A replay window in front of everything downstream.',
    what: 'A distributed append-only log. Producers write to partitioned topics, consumers track their own offsets, and messages stay for a configured retention window regardless of who has read them.',
    why: 'Sits between the sources and the bronze layer. Because consumers hold their own offset and the log keeps the data anyway, an ingest job can fail for an afternoon and then catch up — rather than losing the afternoon.',
    watch: [
      'It is a lot of machinery for a household. This is on the radar as practice, and it is the one I would drop first if I were being strict.',
      'Retention is a storage bill, not a setting. Work out the window before turning on a firehose.',
      'Mosquitto already carries the household sensor traffic. Two buses is one bus too many unless the second earns it.',
    ],
    links: [
      { label: 'kafka.apache.org', href: 'https://kafka.apache.org/documentation/' },
    ],
    related: ['nats', 'apache-flink', 'mosquitto'],
  },
  {
    slug: 'nats',
    name: 'NATS',
    ...
    movement: 'new',
    since: '2026-09',
    tagline: 'The same job in one binary, if Kafka turns out to be too much.',
    what: 'A messaging system in a single Go binary, with JetStream adding persistence, replay and consumer offsets on top of plain publish-subscribe.',
    why: 'Researching it as the lighter half of a bake-off with Kafka. If JetStream gives me a replay window and durable consumers without a cluster to operate, then Kafka is a lot of machinery to keep for one rack.',
    watch: [
      'The ecosystem around Kafka is bigger. Connectors you assume exist may not.',
      'JetStream is the part that matters here. Plain NATS has no replay, which is the entire reason the buffer exists.',
    ],
    links: [
      { label: 'docs.nats.io', href: 'https://docs.nats.io/' },
    ],
    related: ['apache-kafka', 'mosquitto'],
  },
